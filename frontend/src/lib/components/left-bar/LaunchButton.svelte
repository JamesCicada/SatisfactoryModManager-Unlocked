<script lang="ts">
  ...
  $: {
    ...
    if (info && $hasFetchedMods) {
      ...
      Object.keys($lockfileMods).map(async (modReference) => {
        newCompatibilities[modReference] = await getCompatibility(
          modReference,
          client,
        );
      });
  ...
  $: reportedCompatibilities = Object.entries(compatibilities).reduce(
  ...
  $: versionCompatibilities = Object.entries(compatibilities).reduce(
    (acc, [modReference, compatibility]) => {
  ...
  $: versionIncompatible = Object.keys($lockfileMods).filter(
    (modReference) =>
      versionCompatibilities[modReference]?.state === CompatibilityState.Broken,
  );
  $: versionPossiblyCompatible = Object.keys($lockfileMods).filter(
    (modReference) =>
      versionCompatibilities[modReference]?.state ===
      CompatibilityState.Damaged,
  );
  $: reportedIncompatible = Object.keys($lockfileMods).filter(
    (modReference) =>
      reportedCompatibilities[modReference]?.state ===
      CompatibilityState.Broken,
  );
  $: reportedPossiblyCompatible = Object.keys($lockfileMods).filter(
    (modReference) =>
      reportedCompatibilities[modReference]?.state ===
      CompatibilityState.Damaged,
  );
  ...
  $: launchButtonError =
    versionIncompatible.length > 0 || reportedIncompatible.length > 0;
  $: launchButtonWarning =
    !launchButtonError &&
    (versionPossiblyCompatible.length > 0 ||
      reportedPossiblyCompatible.length > 0);
  ...
  {#if hasSelectedInstall}
    <div class="flex flex-col gap-1 px-1">
      <div class="flex items-center gap-1.5 text-xs">
        <button
          class="btn px-1 py-0 h-auto text-xs flex items-center gap-1.5"
          on:click={() => ($launchDirect = !$launchDirect)}
        >
          <SvgIcon
            class="h-4 w-4"
            icon={$launchDirect
          ...
          />
          <T
            defaultValue="Direct Launch exe"
            keyName="launch-button.direct-launch"
          />
          <span
            class="cursor-help text-surface-500"
            use:popup={directLaunchTooltip}
          >
            <SvgIcon class="h-3.5 w-3.5" icon={mdiInformationOutline} />
          </span>
        </button>
      </div>
      <Tooltip class="!mt-0" popupId={"direct-launch-info"}>
        <T
          defaultValue="Attempt to run the game directly from the exe file"
          keyName="launch-button.direct-launch-tooltip"
        />
      </Tooltip>
      {#if $launchDirect}
        <div class="flex items-center gap-1 max-w-full">
          <span class="text-xs truncate flex-auto text-surface-500"
            >{$launchDirectExe || "No executable selected"}</span
          >
          <button class="btn btn-xs px-1.5" on:click={browseExe}>
            <SvgIcon class="h-3.5 w-3.5" icon={mdiFileSearch} />
          </button>
          {#if $launchDirectExe}
            <button class="btn btn-xs px-1.5" on:click={clearExe}>
              <SvgIcon class="h-3.5 w-3.5" icon={mdiClose} />
            </button>
          {/if}
        </div>
      {/if}
  ...
  <center use:popup={popupHover}>
    {#if areOperationsQueued}
      ...
      >
        <Marquee class="flex-auto text-start">
          <T
            defaultValue={"Apply {queued, plural, one {one change} other {# changes}}"}
            keyName="launch-button.apply-queued"
            params={{ queued: $queuedMods.length }}
          />
        </Marquee>
        <SvgIcon class="h-5 w-5" icon={mdiTrayFull} />
      </button>
    {:else if $hasPendingProfileChange}
      ...
      >
        <span>
          <T
            defaultValue={"Apply {profile}"}
            keyName="launch-button.apply-profile-change"
    ...
    {:else if !isInstallLaunchable}
      <button class="btn h-8 w-full text-sm bg-surface-200-700-token" disabled>
        <Marquee class="flex-auto text-start">
          <T
            defaultValue="SMM can't launch this install"
            keyName="launch-button.cant-launch"
          />
        </Marquee>
      </button>
    {:else if $launchButton === "normal" || $isGameRunning || $isLaunchingGame}
      ...
      >
        ...
        <SvgIcon class="h-5 w-5" icon={mdiOpenInNew} />
      </button>
    {:else if $launchButton === "cat"}
      ...
      >
        ...
        />
        <div role="button" tabindex="0" on:mousedown={catMouseDown}>
          <img
  ...
  {#if versionIncompatible.length > 0 || versionPossiblyCompatible.length > 0 || reportedIncompatible.length > 0 || reportedPossiblyCompatible.length > 0}
    <span>
      <T
        ...
        keyName="launch-button.you-have-warning-mods"
      />
    </span>
    <ul class="list-disc pl-5">
      {#if versionIncompatible.length > 0}
        <li>
          <span>
            <T
              defaultValue={"{versionIncompatible} incompatible {versionIncompatible, plural, one {mod} other {mods}} which will not load or crash our game"}
              keyName="launch-button.incompatible-mods"
              params={{ versionIncompatible: versionIncompatible.length }}
            />
          </span>
        </li>
      {/if}
        ...
        <li>
          <span>
            <T
              ...
              params={{
                versionPossiblyCompatible: versionPossiblyCompatible.length,
              }}
            />
          </span>
        </li>
        ...
        <li>
          <span>
            <T
              ...
              params={{
                reportedPossiblyCompatible: reportedPossiblyCompatible.length,
              }}
            />
          </span>
        </li>
    ...
    <span>
      <T
        defaultValue="Are you sure you want to launch?"
        keyName="launch-button.are-you-sure-warning"
      />
    </span>
  {:else if areOperationsQueued}
    <span>
      <T
        defaultValue={'Changes have not yet been made to your mod files. Click the button below to apply the changes you have queued.\n\n(You\'re in Queue "Start manually" mode)'}
        keyName="launch-button.changes-queued"
      />
    </span>
  {:else if $hasPendingProfileChange}
    <span>
      <T
        defaultValue={'Changes have not yet been made to your mod files. Click the button below to apply the new profile.\n\n(You\'re in Queue "Start manually" mode)'}
        keyName="launch-button.profile-change-queued"
      />
    </span>
  {:else if $isGameRunning}
    <span>
      <T
        defaultValue="Your game launcher is reporting that the game is already running (or still in the process of closing)."
        keyName="launch-button.game-running"
      />
    </span>
  {:else if $isLaunchingGame}
    <span>
      <T
        defaultValue="Launch in progress..."
        keyName="launch-button.launch-in-progress"
      />
    </span>
  {:else if !!$progress}
    <span>
      <T
        defaultValue="An operation is already in progress."
        keyName="launch-button.operation-in-progress"
      />
    </span>
  {:else if !isInstallLaunchable}
    <span>
      <T
        defaultValue="The Mod Manager is not capable of launching this install type, but it will still manage the mod files for you. Launch Satisfactory using your usual game launcher."
        keyName="launch-button.cant-launch-tooltip"
      />
    </span>
  {:else}
    <span>
      <T
        defaultValue="You're ready to rumble!\n\nNote: The Mod Manager has already finished installing the mod files for you. You could launch the game using your usual game launcher and mods would still be loaded."
        keyName="launch-button.ready"
      />
    </span>
  {/if}
</Tooltip>
